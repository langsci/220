...
\newsavebox\ExEighteenbox
\setbox0=\hbox{%%https://tex.stackexchange.com/a/366710/102558
\begin{tikzpicture}[baseline,anchor=base,inner ysep=0pt]
\matrix[inner ysep=0pt,matrix of nodes,nodes={anchor=base west,baseline},row sep=1cm] (DiesselEx18) {%
{[\textsc{verb}} & {\textsc{[verb]\textsubscript{obj}]\textsubscript{vp}}} &[2cm] {\textsc{[[verb]\textsubscript{obj}}} & {\textsc{verb]\textsubscript{vp}}} \\
{\textsc{[aux}}  & {\textsc{verb]\textsubscript{vp}}}  &  {\textsc{[verb}}  & {\textsc{aux]\textsubscript{vp}}}\\};
\foreach \i in {1,...,4} \draw[-{Triangle[]}] let 
                                      \p1 = (DiesselEx18-1-\i.south),
                                      \p2 = (DiesselEx18-2-\i)
                                  in (\x2,\y1) -- (DiesselEx18-2-\i);
\end{tikzpicture}}
\sbox\ExEighteenbox{\copy0}
\ea\label{ex:key:18}
\attop{\vspace{-\baselineskip}\usebox\ExEighteenbox}
\z
...
\newsavebox\ExNineteenbox
\setbox0=\hbox{%%https://tex.stackexchange.com/a/366710/102558
\begin{tikzpicture}[baseline,anchor=base,inner ysep=0pt]
\matrix[inner ysep=0pt,matrix of nodes,nodes={anchor=base west,baseline},row sep=1cm] (DiesselEx19) {%
{[[[ \textsc{np} ]}  &   {\textsc{p]\textsubscript{pp}}} & {[ \textsc{… v …]\textsubscript{s}}} \\
{\textsc{[[…s…}} & {\textsc{sub]\textsubscript{ac}}} &  {[ \textsc{… v …]\textsubscript{mc}}]}\\};
\foreach \i in {1,...,3} \draw[-{Triangle[]}] let 
                                      \p1 = (DiesselEx19-1-\i.south),
                                      \p2 = (DiesselEx19-2-\i.north)
                                  in (DiesselEx19-1-\i) -- (\x1,\y2);
\end{tikzpicture}}
\sbox\ExNineteenbox{\copy0}
\ea\label{ex:key:19}
\attop{\vspace{-\baselineskip}\usebox\ExNineteenbox}
\z
...
\newsavebox\ExTwentybox
\setbox0=\hbox{%%https://tex.stackexchange.com/a/366710/102558
\begin{tikzpicture}[baseline,anchor=base,inner ysep=0pt]
\matrix[inner ysep=0pt,matrix of nodes,nodes={anchor=base west,baseline},row sep=1cm] (DiesselEx20) {%
...
\foreach \i in {1,...,3} \draw[-{Triangle[]}] let 
                                      \p1 = (DiesselEx20-1-\i.south),
                                      \p2 = (DiesselEx20-2-\i)
                                  in (\x2,\y1) -- (DiesselEx20-2-\i);
\end{tikzpicture}}
\sbox\ExTwentybox{\copy0}                                  
\ea\label{ex:key:20}
\attop{\vspace{-\baselineskip}\usebox\ExTwentybox}
\todo[inline]{needs fixing}
\z
