...
give → for\todo{should these be headings of some sort?}
...
\todo[inline]{do we need `a.' and `b.' (all tables)?}
...
\begin{tabularx}{\textwidth}{llX lll} 
\lsptoprule
...
The fourth SVO\& GenN language exhibiting a similar pattern is Jabem, an Oceanic language in the Austronesian family spoken in Papua New Guinea. In \tabref{extab:dryer:23} is a list of the prepositions and postpositions of Jabem (\citealt{Dempwolff1939}; \citealt{BradshawCzobor2005}: 42–44; \citealt{Ross2002}: 291).
...
\clearpage 
The languages illustrated in \REF{ex:dryer:11} to \tabref{extab:dryer:29} \todo{check uniformity. Maybe \tabref{extab:dryer:16}?}
...
In \tabref{extab:dryer:35} is a list of prepositions and postpositions of Kanuri (\citealt{Hutchison1981}: 257–263).
...
\todo[inline]{can we use $\emptyset$ or $\bar{x}$ instead of `mean' (all tables)?}
\end{table}
...
\lspbottomrule
\end{tabularx}
\todo[inline]{last column header total->mean?}
