...
\isi{pronouns}). Conversely, \isi{ergative} \isi{alignment}  is sometimes restricted to a right end portion of the hierarchy\is{referential hierarchy} (e.g. inanimate\is{animacy}
nouns, nouns as opposed to \isi{pronouns}, nouns and 3rd \isi{person} \isi{pronouns}), but is usually not restricted to a left end portion of the hierarchy
...
In other cases, the \isi{ergative} marker is derived from a marker used to encode \isi{instrument}s in \isi{transitive} sentences with no overt third \isi{person} arguments. In these sentences, the
  \isi{instrument} can be reinterpreted\is{reanalysis} as an \isi{agent}, thus evolving into the A
 ...
 marker originally used for the \isi{instrument} becomes an \isi{ergative} marker. This process has
  ...
  \refP{hanis}.  \isi{Instrument}s are typically inanimate\is{animacy}, so the relevant markers 
...
marker for a co-occurring argument. \isi{Topic} markers
...
for a co-occurring A argument.  This is a metonymization\is{metonymy} process triggered by the
  ...
  construction, for example, \isi{instrument}s can be reinterpreted\is{reanalysis}
...
distinctions for \isi{pronouns}, but not for nouns
