...
For example, a \isi{number} of \isi{word order} correlations have been explained by assuming that speakers will recurrently select particular word orders as opposed to others because these orders lead to syntactic structures that are easier to process (\citealt{Hawkins2004},  among others). Another \isi{case} in point is provided by explanations of the use of explicit marking for different grammatical meanings, for example the use of overt marking for different \isi{number} values, or that of dedicated \isi{case} marking for different NP types occurring in particular argument roles. Cross-linguistically, explicit marking may be restricted to less frequent meanings, for example \isi{plural} rather than \isi{singular}, \isi{animate} rather than
in\isi{animate} P arguments, or inanimate rather than \isi{animate} A arguments, but
is usually not restricted to more frequent meanings.\footnote{Following a standard
    ...
    the only argument of intransitive verbs.} This has been assumed to reflect a principle of \isi{economy} whereby speakers will tend to use explicit marking only when they really need to do so. Explicit marking can be restricted to less frequent meanings because more frequent ones are easier to identify, and hence less in need to be disambiguated
...
Second, individual configurations arise through several distinct
  ...
  \isi{general} principle. This challenges the idea that explanations for
...
\exNG{hierarchy}{1st \isi{person} pronouns  $>$ 2nd \isi{person} pronouns $>$ 3rd \isi{person} pronouns
 $>$ human $>$ \isi{animate}
...
\isi{accusative} and \isi{ergative} \isi{case} marking \isi{alignment}
 ...
 Accusative \isi{alignment}
  can be restricted to a left end portion of the hierarchy (e.g.  pronouns, human and \isi{animate}
nouns), but is usually not restricted to a right end portion of the hierarchy (e.g. inanimate nouns, nouns as opposed to
pronouns). Conversely, \isi{ergative} \isi{alignment}  is sometimes restricted to a right end portion of the hierarchy (e.g. inanimate
nouns, nouns as opposed to pronouns, nouns and 3rd \isi{person} pronouns), but is usually not restricted to a left end portion of the hierarchy
(1st/2nd \isi{person} pronouns, pronouns as opposed to nouns, pronouns and \isi{animate} nouns).
...
need of \isi{disambiguation}. The NPs towards the right end of the
...
easy to identify, and hence less in need of \isi{disambiguation}. Dedicated
\isi{case} marking for P arguments, leading to \isi{accusative} \isi{alignment}, may
then be limited to the NPs towards the left end of the hierarchy
(pronouns, \isi{animate} nouns). By contrast, these NPs are more likely to
occur as A arguments, hence, when they do, the A role is less in need
of \isi{disambiguation}. Dedicated \isi{case} marking for A arguments, leading to
\isi{ergative} \isi{alignment}, may then be limited to the NPs towards the right
...
In many cases where \isi{accusative} or \isi{ergative} \isi{alignment} is restricted to
particular NP types, the relevant \isi{alignment} pattern is a result of the
development of an \isi{accusative} or \isi{ergative}
marker through the reinterpretation of a pre-existing
element with similar distributional restrictions.  In some cases, for example, \isi{accusative} markers restricted
to pronominal, \isi{animate} or  \isi{definite} direct objects are 
structurally identical to \isi{topic} markers. This is illustrated  in
\refP{kanuri} for \ili{Kanuri}. 
...
In such cases, the \isi{accusative} marker
plausibly originates from the \isi{topic}
marker in contexts where the latter refers to a P argument and  is
reinterpreted as a marker for this argument (`As for X' $>$ `X ACC': see, for example, \citealt{Rohlfs1984} and \citealt{Pensado1995} for \ili{Romance}
languages, and \citealt{Konig2008} for several African languages). 
Topics are usually pronominal, \isi{animate} and
\isi{definite}, so \isi{topic} markers
are mainly used in the same contexts as the resulting \isi{accusative}
markers. 
...
referent is a new or unexpected \isi{agent}. This strategy does not
    apply to  first and second \isi{person} pronouns because the referents
...
In other cases, the \isi{ergative} marker is derived from a marker used to encode instruments
 in  transitive sentences with no overt third \isi{person} arguments. In these sentences, the
  \isi{instrument} can be reinterpreted as an \isi{agent}, thus evolving into the A
  argument of the sentence. As a result,  the
 marker originally used for the \isi{instrument} becomes an \isi{ergative} marker. This process has
  been reconstructed by \citet{Mithun2005} for \ili{Hanis Coos}, illustrated in
  \refP{hanis}.  Instruments are typically inanimate, so the relevant markers 
do not usually occur with first and second \isi{person} pronouns.
...
however, underwent phonological reduction, plausibly due to their high discourse \isi{frequency}. As a result, as can be seen from Table \ref{apics}, pronouns developed
 distinct forms for A and S
    ...
    \isi{alignment} pattern restricted to pronouns \citep{HaspelmathAPiCS}.
...
restricted to roles more in need of \isi{disambiguation}. 
In some cases, a
...
demonstratives and third \isi{person} pronouns are reinterpreted as markers
...
phonological reduction due to their discourse \isi{frequency}. 
...
developmental mechanisms involved are not \isi{subject} to particular
...
for its former
       P argument (\citealt{Lord1993,Chappell2013}, among
         several others). The P arguments of `take' verbs can be pronominal, nominal, \isi{animate} or inanimate (e.g. `take him, it, the child, the sword'), 
...
where the \isi{accusative} marker {\em de},
         derived from a `take' verb, applies to both \isi{animate} and inanimate P arguments.
...
Accusative and \isi{ergative} markers  can  also develop from the
  \isi{reanalysis} of possessor or \isi{oblique} markers used on the notional A or
  ...
  `X is occupied with the Verbing  {\bf of} Y $>$ `X
     ...
     \citealt{HarrisCampbell1995,Bubenik1998,Gildea1998,Creissels2008}). In such cases
        ...
        pronominal, \isi{animate} or inanimate NPs (e.g. `The Verbing of
        ...
        with nominal inanimate and pronominal \isi{animate} arguments.
...
opposed to pronouns, leading to \isi{accusative} \isi{alignment} becoming
restricted to pronouns. In  \ili{Nganasan}, however, a combination of sound change and
\isi{analogical levelling} led to a  loss of inflectional
...
of \isi{disambiguation} than pronominal ones.
...
\ili{Nganasan} (Uralic; \fatcitNP{Filimonova2005}{94})\\
  \ea
    ...
    \glt `You have taken me with you.' (pronominals originally had dedicated \isi{accusative} forms, e.g. {\em mənə-m} `1\textsc{sg-acc}')
...
marking for \isi{singular} and \isi{plural}. Languages can use overt marking for \isi{plural} and \isi{zero marking} for
  ...
  only for meanings that are more in need of \isi{disambiguation}, and \isi{plural} is more in need of \isi{disambiguation} than \isi{singular} due to its
...
supported by a \isi{number} of diachronic processes that lead languages to
have zero \isi{marked} singulars and overtly \isi{marked} plurals.


Often, in languages which make no distinction between \isi{singular}
and \isi{plural}, an overt \isi{plural} marker evolves through the reinterpretation of
pre-existing expressions, whereas
    singulars retain \isi{zero marking}. Sometimes, some expression takes on a \isi{plural} meaning originally associated with
...
marker through regular sound change in a situation where both \isi{singular}
and \isi{plural} are originally overtly \isi{marked}. This was, for example, the \isi{case} in
\ili{English}, where \isi{singular} and \isi{plural} were both originally overtly
  \isi{marked} in most cases. The current
configuration with zero \isi{marked} singulars and {\em -s} \isi{marked} plurals
...
disambiguate \isi{plural} as opposed to \isi{singular}. In some cases, an overt
\isi{plural} marker arises as a result of a metonymization
process whereby \isi{plural} meaning is transfered from a quantifier to some
...
the \isi{case} of \isi{accusative} and \isi{ergative} \isi{case} marking \isi{alignment}, this point
...
A \isi{case} in point is provided by Kxoe,
illustrated in Table \ref{kxoe} below. This language has \isi{gender}
markers derived from third \isi{person} pronouns \citep{Heine1982}. As the
...
quantifier. In expressions where the quantifier is \isi{singular} (`one of
...
is overtly \isi{marked}. This was the \isi{case} in \ili{Imonda}, which has zero \isi{marked} plurals, but developed
an overt non-\isi{plural}
(\isi{singular} and \isi{dual}) marker
 from a source \isi{case} marker used in \isi{partitive} constructions \fatcit{Imonda}{38--39}.
...
Similar observations apply to loss of \isi{number} markers through sound
change. This process can affect
either \isi{singular} or \isi{plural} markers depending on the phonological
properties of the marker. From one language to another, then, the process may lead either to  zero \isi{marked} singulars and
overtly \isi{marked} plurals, as detailed above for
\ili{English}, or to the opposite configuration. In the
\ili{Indo-Aryan} language \ili{Sinhala},  for example, some inanimate nouns have
overtly \isi{marked} singulars and
zero \isi{marked} plurals
 (e.g. {\em pot-\textbf{\textit {a}}/pot} `book-\textsc{sg}/book.\textsc{pl}'). This was a result of
 sound changes leading to the loss of the \isi{plural} ending of a specific inflectional class in the ancestor language
% specific inflectional class in middle indica (declension ii, which is the ancestor of today's inanimate classes, e.g. at-a SG at PL 'hand, 256)
 ...
 language, nouns in classes 3/4 have overt
  ...
  Originally, both \isi{singular} and \isi{plural} were \isi{marked} overtly through the
...
reasons.  
In many cases, both \isi{singular} and \isi{plural} are originally zero
\isi{marked} (i.e., the language makes no distinction between the two), but
...
\isi{singular} and \isi{plural} are originally overtly \isi{marked}, and sound change
...
overtly \isi{marked} and \isi{plural} is zero \isi{marked} is assumed to be due to the
...
those traits are adaptive to the \isi{environment}, in the sense of conferring an evolutionary advantage to the organisms
...
for some phenomenon X can be framed as a logical \isi{implication},  X
$\rightarrow$ A (because X will always involve A, unless other
motivations for X are also postulated).  This
\isi{implication} means, however, that the absence of A will lead to phenomena different
...
\isi{implication}, $\sim$X $\rightarrow$ A, with a different truth table.
For example, if the non-occurrence
of configurations where \isi{singular} is
overtly \isi{marked} and \isi{plural} is zero \isi{marked} (X)  is assumed to be due to
...
the latter phenomenon is also due to \isi{economy}. This undermines the \isi{general}
...
the equivalent of the notion of adaptive \isi{evolution} through natural
\isi{selection} in evolutionary
...
distribution in a population.\footnote{A referee suggests that this is
  ...
  selected over others, as is the \isi{case} with  result-oriented
  explanations of typological universals and explanations of
  biological \isi{evolution} in terms of adaptiveness through natural \isi{selection}.}
...
source constructions or developmental mechanisms, it could be the \isi{case}
...
marking for singulars (\isi{zero marking}
becoming restricted to \isi{singular} due to the development of an overt \isi{plural}
marker, phonological erosion of an existing overt \isi{singular}
...
particular source constructions or developmental mechanisms, any differences in the \isi{frequency} of particular
configurations will reflect differences in the \isi{frequency} of
the source constructions or developmental mechanisms that give
rise to those configurations. The higher
\isi{frequency} of particular configurations will then  be a result of the higher \isi{frequency} of the  source
...
\isi{general}. These views are manifested, for example, in accounts of the
...
this \isi{case} one has to account for why these principles are violated in
...
example, the idea that \isi{zero marking} of more frequent meanings is
motivated by \isi{economy} is in constrast with the fact that these meanings
are overtly \isi{marked} in many languages. 

These facts have sometimes been
dealt with in terms of \isi{competing motivations} models, but a \isi{general}
