...
As for \isi{propagation} processes, Hawkins\ia{Hawkins} is usually reticent with regard to the forces that implement efficient\is{efficiency} structures, even though the central diachronic mechanism in his programme is that of “\isi{selection}”: Efficient\is{efficiency} variants are said to be selected\is{selection} relatively more frequently\is{frequency} than their inefficient\is{efficiency} counterparts, until they may ultimately oust the inefficient\is{efficiency} ones completely. It is in this way, Hawkins argues, that preferred patterns in performance can conventionalize into grammatical rules, although he concedes in \citet[10]{Hawkins2014_VarEff} that it is presently poorly understood how exactly this “translation from performance to grammar” works. Now, if one subscribes to the view that \isi{propagation} is entirely driven by sociolinguistic forces like prestige, solidarity and the resulting \isi{accommodation} (e.g. \citealt{Croft2000_Change}; \citealt{Cristofaro2017_Dep}; \citealtv{chapters/cristofaro}), it remains mysterious, indeed, how Hawkins’s very idea of \isi{selection} processes can fit in. 
